...
namespace _05.GreedyTimes
{
    public class StartUp
    {
        public static void Main()
        {
            ...
            FillUpBag(input, bag, bagCapacity);

            PrintOutTreasures(bag);
        }

        private static void PrintOutTreasures(Dictionary<string, Dictionary<string, long>> bag)
        {
            foreach (KeyValuePair<string, Dictionary<string, long>> item in bag.Where(b => b.Value.Count > 0)
                .OrderByDescending(i => i.Value.Sum(x => x.Value)))
            {
        ...
        private static void FillUpBag(string[] input, Dictionary<string, Dictionary<string, long>> bag, long bagCapacity)
        {
