...
public class Topping
{
    ...
    {
        this.Type = type;
        ValidateWeiht(type, weight);
        this.Weight = weight;
    }

    private double TypeMultiplier => validToppings[type];

    public double Calories => DEFAULT_MULTIPLIER * this.Weight * TypeMultiplier;

    private void ValidateWeiht(string type, double weight)
    {
    ...
    public string Type
    {
        ...
        set
        {
            if (!this.validToppings.Any(t => t.Key == value.ToLower()))
            {
