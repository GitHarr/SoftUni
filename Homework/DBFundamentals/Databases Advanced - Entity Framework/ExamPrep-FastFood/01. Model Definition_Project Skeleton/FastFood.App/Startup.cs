...
namespace FastFood.App
{
	public class Startup
	{
		...
		private static void ImportEntities
            (FastFoodDbContext context, string baseDir = @"../../../..\Datasets\")
		{
			const string exportDir = "../../../ImportResults/";

			var employees = DataProcessor.Deserializer.ImportEmployees
                (context, File.ReadAllText(baseDir + "employees.json"));
			PrintAndExportEntityToFile(employees, exportDir + "Employees.txt");

			var items = DataProcessor.Deserializer.ImportItems
                (context, File.ReadAllText(baseDir + "items.json"));
