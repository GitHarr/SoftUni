namespace PetClinic.App
{
    ...
    public class StartUp
    {
        ...
        private static void ImportEntities
            (PetClinicContext context, string baseDir = @"..\..\..\Datasets\")
        {
            const string exportDir = "../../../Results/";

            string animalAids = DataProcessor.Deserializer.ImportAnimalAids
                                (context, File.ReadAllText(baseDir + "animalAids.json"));
            PrintAndExportEntityToFile(animalAids, exportDir + "AnimalAidsImport.txt");

            string animals = DataProcessor.Deserializer.ImportAnimals
                             (context, File.ReadAllText(baseDir + "animals.json"));
            PrintAndExportEntityToFile(animals, exportDir + "AnimalsImport.txt");

            string vets = DataProcessor.Deserializer.ImportVets
                          (context, File.ReadAllText(baseDir + "vets.xml"));
            PrintAndExportEntityToFile(vets, exportDir + "VetsImport.txt");

            var procedures = DataProcessor.Deserializer.ImportProcedures
                             (context, File.ReadAllText(baseDir + "procedures.xml"));
        ...
        private static void ExportEntities(PetClinicContext context)
        {
            ...
            string animalsExport = DataProcessor.Serializer.ExportAnimalsByOwnerPhoneNumber
                                   (context, "0887446123");
        ...
        {
            var bonusOutput = DataProcessor.Bonus.UpdateVetProfession
                              (context, "+359284566778", "Primary Care");
